...
exports.superAdminLogin = async (req, res, next) => {
  ...
  console.log('superAdminLogin:', req.body);
  try {
    ...
    if (
      ...
      !user.role.privileges.some(p => rolePrivileges.includes(p))
    ) {
      return res.status(403).json({ error: "Forbidden: Super Admin only" });
    }


    console.log('superAdminLogin: user role privileges:', user.role.privileges);
